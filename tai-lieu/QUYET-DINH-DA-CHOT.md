...
| 18.58 | **Thông báo nhắc học MẶC ĐỊNH BẬT** (trước đây mặc định tắt, 18.40); người học tắt được trong Cài đặt. Tài khoản chưa từng tắt (`thongBao.bat` không phải `false`) được coi là bật. (1) **Hỏi quyền lần đầu**: máy chưa từng được hỏi thì ~1.5 giây sau khi mở app hiện hộp "Cho phép Riyi nhắc bạn học?" (nói rõ 3 khung giờ, tắt được trong Cài đặt), nút "Cho phép" gọi hộp xin quyền của trình duyệt NGAY trong lần bấm (trình duyệt, nhất là iPhone, không cho tự bật hộp xin quyền khi người dùng chưa bấm gì), "Để sau" thì thôi. Mỗi máy chỉ hỏi MỘT lần (localStorage). Không hỏi khi: trình duyệt không hỗ trợ (iPhone chưa cài app vào màn hình chính), máy đã cho phép hoặc đã chặn, hoặc tài khoản đã tự tắt. Khách cũng được hỏi; cho phép rồi thì đăng nhập xong tự được đăng ký. (2) **Tự đăng ký lại** mỗi lần mở app (`dangKyLaiNeuCan`): đã đăng nhập + không tự tắt + máy đã cho phép thì lưu mã thiết bị; chỉ ghi Firestore khi mã khác lần trước. (3) Cài đặt: công tắc chỉ hiện BẬT khi tài khoản không tắt VÀ máy này đã cho phép; máy đang chặn thông báo thì hiện dòng hướng dẫn mở lại quyền trong cài đặt trình duyệt/điện thoại; mô tả thêm "Mặc định bật". Tắt vẫn như cũ: tắt cho cả tài khoản (mọi máy) |
| 18.59 | **Thi thử HSK** (Khám phá → Thi thử HSK; bỏ nhãn "Sắp có"). Đề gốc chủ dự án tải về và đã kiểm tra bản quyền, để ở `de_thi_HSK/` (pdf + mp3 cùng tên, KHÔNG đưa lên GitHub). Làm thử đề **H10901 (HSK 1, 40 câu)**, duyệt xong mới dựng các đề còn lại. **Công cụ** `python cong-cu/dung-de-thi.py <MÃ>` (cần PyMuPDF) đọc PDF: chữ Hán, pinyin (ghép theo vị trí, đặt trên CẢ TỪ đúng như đề in), ảnh từng câu (cắt theo khung ảnh, gộp ảnh bị tách đôi, bỏ logo chân trang của trang nguồn), đáp án (trang 卷答案), lời thoại phần nghe (听力材料) → `public/du-lieu/thi-thu/<MÃ>/de.json` + `hinh/` + `nghe.mp3`, và `danh-sach.json`; báo lỗi nếu không đủ 40 câu. Hiện chỉ hiểu bố cục HSK 1. **Luật làm bài** (chủ dự án chốt): mỗi phần một trang cuộn, cuối trang có nút sang phần sau, quay lại phần trước sửa đáp án được; phần Nghe có nút nghe ở đầu trang, phát MỘT MẠCH, không dừng/tua/nghe lại (tua lùi bị đưa về chỗ xa nhất đã nghe), thoát ra hay tải lại trang thì lần sau **nghe tiếp từ chỗ dừng**, nghe hết mới sang phần Đọc (trừ khi không tải được file nghe), sau đó nút nghe khoá; phần Đọc đếm ngược theo đề (HSK 1: 17 phút) tính từ lúc vào trang Đọc lần đầu, vẫn chạy khi thoát ra, hết giờ tự nộp; bài làm dở (đáp án, trang, giây đã nghe, giờ bắt đầu Đọc) lưu trên máy (localStorage). **Nộp bài**: hỏi lại (báo số câu bỏ trống), chấm theo thang HSK (HSK 1: 5 điểm/câu, mỗi phần 100, tổng 200, đạt ≥ 120), xem lại từng câu (đúng/sai bằng chữ + ✓ ✗, đáp án, lời thoại phần nghe), làm lại đề. Người đăng nhập: điểm lưu ở `nguoiDung.thiThu.<MÃ>` = {lanCuoi, cao, soLan} (firestore.rules thêm khoá `thiThu`); khách làm được nhưng không lưu. **Giữ đúng đề gốc** (chủ dự án chốt): chỗ nào đề có pinyin thì hiện, không có (tiêu đề 第一部分, lời thoại) thì không thêm — ngoại lệ có chủ đích với quy tắc "chữ Hán luôn có pinyin". App chỉ thêm một dòng hướng dẫn tiếng Việt nhỏ dưới mỗi phần. File nghe HSK 1 gốc chỉ 1,75 MB nên không cần nén |
| 18.60 | **Đổi luật nghe của Thi thử** (chủ dự án: "liên quan ý thức người học, không cần quá chặt"): thoát ra / tải lại trang / app bị crash khi CHƯA nghe hết thì mở lại được **nghe lại từ đầu** (nút "Nghe lại từ đầu"; không lưu vị trí nghe nữa). Trong cùng một lần mở trang mà bị ngắt (cuộc gọi, khoá màn hình) thì nút "Nghe tiếp từ mm:ss" phát tiếp chỗ dở. Giữ nguyên: trong lúc phát không tua lùi được; nghe hết trọn một lần thì khoá. **Dựng xong 4 đề còn lại** (H10902, H11003, H11004, H11005, đều HSK 1): mỗi đề đủ 40 câu, 44 ảnh; kiểm tra tự động (chữ Hán mọi câu có nguyên văn trong PDF, đủ lựa chọn, đúng 1 chỗ trống ở câu điền từ, đủ 20 lời thoại) và so bằng mắt ảnh A–F với trang gốc. Sửa công cụ: chữ cái A/B/C in lệch cao thấp vẫn nhận đúng dòng chữ của nó. **LƯU Ý đề H11003 câu 17**: phương án A trong file PDF chỉ có số "0" màu xanh (khác hẳn kiểu chữ, có vẻ file bị sửa hỏng); app giữ nguyên như PDF, KHÔNG tự đoán. Đáp án câu này là C (7 年, khớp lời thoại "工作了七年") nên không ảnh hưởng chấm điểm; chờ chủ dự án kiểm tra lại bản gốc |
| 18.61 | **Thi thử đủ HSK 1–6: 30 đề** (mỗi cấp 5 đề; chủ dự án bỏ đề vào `de_thi_HSK/HSK1…HSK6/`, vẫn không đưa lên GitHub). Phạm vi bài học hằng ngày vẫn là HSK 1–3; riêng Thi thử có tới HSK 6 theo yêu cầu chủ dự án. **Chủ dự án chốt phần Viết (HSK 3–6):** (1) sắp xếp từ thành câu = bấm các mảnh từ theo thứ tự, chấm tự động, đúng nếu trùng MỘT trong các đáp án đề cho (đề ghi "a / b"); (2) HSK 3 viết chữ theo pinyin = gõ chữ (bộ gõ tiếng Trung), chấm tự động; (3) viết tự do (HSK 4 nhìn ảnh đặt câu, HSK 5 viết đoạn 80 chữ, HSK 6 缩写) = có ô viết + đếm chữ, KHÔNG chấm; HSK 4 hiện câu tham khảo của đề khi xem lại; HSK 6 bấm "Bắt đầu đọc" → bài hiện 10 phút (đếm ngược, có nút "Đọc xong") rồi ẩn HẲN mới hiện ô viết, trạng thái lưu cả khi tải lại trang; (4) **tổng điểm CHỈ tính Nghe + Đọc** (mỗi phần quy về 100 theo tỉ lệ câu đúng, tổng 200, đạt ≥ 120), màn kết quả và danh sách đề ghi rõ điều này; phần Viết hiện riêng "đúng x/y câu chấm được, n câu viết tự do không chấm". HSK 1 vẫn ra đúng 5 điểm/câu như trước. **Làm bài:** 3 trang Nghe → Đọc → Viết, mỗi trang có nút sang phần sau và quay lại phần trước (sửa đáp án); một đồng hồ chung cho Đọc + Viết (tổng thời gian ghi ở bìa đề, ví dụ HSK 3: 30 + 15 = 45 phút), hết giờ tự nộp; danh sách đề chia theo cấp, ghi số câu và số phút. **Công cụ** `cong-cu/dung-de-thi.py tat-ca` đọc cả 6 cấp: HSK 1–2 có pinyin (ghép theo vị trí TỪNG KÝ TỰ vì có đề gộp pinyin nhiều chữ vào một mảnh; HSK 1 ra giống hệt bản cũ), HSK 3–6 đọc theo dòng (nhận trang in 2 cột, chữ cái lựa chọn dính liền 《, đoạn văn dùng chung nhiều câu, câu ★ không đánh số, ô trống đánh số trong bài 完形填空 / 选句填空, đoạn văn tách theo chỗ thụt đầu dòng), đáp án phần Viết có nhiều cách đúng, lời thoại gắn cả đoạn nghe chung. Kiểm tra: 30/30 đề đủ số câu như bìa đề; chữ Hán từng câu có nguyên văn trong PDF (7 cảnh báo ở H21004/H21005 là do chữ số trong PDF lưu lệch thứ tự, đã so bằng ảnh: đúng); mảnh từ ghép lại đúng bộ chữ của đáp án; ảnh A–F so tay với trang gốc; test trình duyệt 1 đề mỗi cấp: điền đáp án đúng → 200/200 và Viết đúng hết, không lỗi trang. Dung lượng thêm ≈ 100 MB (file nghe 3–4,3 MB/đề, không cần nén) |
| 18.62 | **Sửa tay đề H11003 câu 17, phương án A** (chủ dự án cho sửa, khép lại lưu ý ở 18.60): PDF gốc chỉ có số "0" màu xanh (file bị hỏng) → thay bằng "5 年" (pinyin "nián" trên 年, cùng kiểu B "6 年", C "7 年"), là một phương án SAI hợp lý; đáp án đúng vẫn là C (lời thoại "工作了七年"). Sửa nằm trong bảng `SUA_TAY` của `cong-cu/dung-de-thi.py` để dựng lại đề không bị mất; chỗ nào PDF hỏng mà chủ dự án cho sửa thì thêm vào bảng này |
| 18.63 | **Chuẩn nội dung của CẢ HAI PHẦN là đại cương thi HSK bản 11/2025** (新版HSK考试大纲, áp dụng từ 7/2026: cấp 1/2/3 = 300/200/500 từ, 246/125/284 chữ, 24/23/23 điểm ngữ pháp). Chủ dự án từng yêu cầu "HSK 3.0" và phần 2 ghi "HSK 3.0 bản 2021"; sau khi được giải thích (bộ tiêu chuẩn 2021 《国际中文教育中文水平等级标准》 có 500/772/973 từ, còn phần 1 đang theo file PDF 2025 do chính chủ dự án cung cấp) đã chốt GIỮ bản 2025 cho cả phần 1 và phần 2 (Tiếng Trung cho người mới bắt đầu), dùng lại toàn bộ dữ liệu đã có. CLAUDE.md sửa lại cho rõ, không còn ghi chung chung "HSK 3.0". Chưa chắc chắn: bản 2025 có được chính thức gọi là "HSK 3.0" không, vì sao số từ cấp 1–3 giảm so với bản 2021 |
| 18.64 | **Phần 2 (người mới) xây quanh giáo trình HSK标准教程 (HSK Standard Course)**. Chủ dự án cung cấp HSK 1 ở `Giao_trinh/HSK1/` (sách giáo khoa 150 trang + sách bài tập 139 trang, đều là ẢNH SCAN không có lớp chữ; 124 file mp3 chia theo bài). Chủ dự án chốt: dùng sách làm **khung tham khảo chính** (thứ tự bài, chủ đề, tiến độ pinyin – chữ – ngữ pháp, văn hóa) VÀ được dùng **chắt lọc** hội thoại, bài tập, âm thanh của sách trong app (không đưa toàn bộ); chủ dự án xác nhận cách dùng này hợp lệ. Bỏ qua hoàn toàn watermark / số điện thoại của trang nguồn (cắt bỏ khi lấy ảnh, không nhắc tới). Thư mục `Giao_trinh/` KHÔNG đưa lên GitHub (.gitignore). Đo được: sách (bản 2014, theo HSK 2009) có 181 từ, phủ **168/300 từ HSK 1 bản 2025 (56%)**; còn thiếu 132 từ phải bổ sung; 7 từ của sách nay thuộc HSK 2 |
| 18.65 | Phần 2: khung HSK 1 đi đúng thứ tự 15 bài của HSK标准教程 1, chia 3 chặng (bài 1–5, 6–10, 11–15), mỗi bài 1 tuần 6 buổi (hội thoại · phát âm · từ vựng + từ bổ sung · chữ Hán + ngữ pháp · luyện tập · ôn + kiểm tra). Thêm **Bài 0 = 2 buổi** (thanh điệu so với dấu tiếng Việt, cách dùng app) trước bài 1. 132 từ HSK 1 bản 2025 sách thiếu được gắn vào bài có chủ đề khớp. Chủ dự án sẽ gửi HSK标准教程 2, 3 trong 2–3 ngày; HSK 4–6 thêm sau. Âm thanh sách HSK 1: giáo khoa ~61 phút / 42 MB (96 kbps), bài tập ~201 phút / 184 MB (128 kbps), phải nén trước khi đưa lên (hạn mức tải 360 MB/ngày) |
